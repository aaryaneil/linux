...
/*MSRs according to the assignment*/
...
/* This structure stores the bit number and 
description, these are the capabilities of the VMX*/
...
/*Pinbased capability of VMX*/

struct info pinbased[5]={
...
struct info procbased_ctls2[23] =
{
...
void report_capability(struct info *cap, uint8_t len, uint32_t lo, uint32_t hi)
{
	uint8_t i;
	struct info *c;
	char msg[MAX_MSG];
...
void detect_vmx_features(void)
{
	...
	{
	pr_info("No capability set for “Activate Secondary Controls” control in the primary procbased controls");
	}
pr_info("\n");
...
{
printk(KERN_INFO "The Module has exited at this point\n");
}
